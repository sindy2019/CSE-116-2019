  ...
  def aimFire(jsonGameState: String): List[Projectile] = {
    ...
    if (players.isEmpty){
      ...
      if (closest_player == null) {
        ...
        var intersection_x = 0.0
        var intersection_y = 0.0
        if (closest_player.velocity.x > 0) {
          intersection_x = closest_player.location.x + distance / dir_edge * closest_player.velocity.x
        } else {
          intersection_x = closest_player.location.x - distance / dir_edge * closest_player.velocity.x
        }

        if (closest_player.velocity.y > 0) {
          intersection_y = closest_player.location.y + distance / dir_edge * closest_player.velocity.y
        } else {
          intersection_y = closest_player.location.y - distance / dir_edge * closest_player.velocity.y
        }
  ...
  def getFitnessFunction(targetPlayer: Player): Double => Double = {
    ...
    (distance : Double) => {
      ...
      if (targetPlayer.velocity.x > 0) {
        intersection_x = targetPlayer.location.x + distance / dir_edge * targetPlayer.velocity.x
      } else {
        intersection_x = targetPlayer.location.x - distance / dir_edge * targetPlayer.velocity.x
      }

      if (targetPlayer.velocity.y > 0) {
        intersection_y = targetPlayer.location.y + distance / dir_edge * targetPlayer.velocity.y
      } else {
        intersection_y = targetPlayer.location.y - distance / dir_edge * targetPlayer.velocity.y
      }
      val intersection = new PhysicsVector(intersection_x, intersection_y)
